...
use Validator;
use App\Mail\SendEmailConnexion;
...
class CustomersController extends Controller
{
    ...
    public function sendEmail(Request $request, $id)
    {
        try {
            ...
            // Récupérer tous les emails associés au client
            $emails = $customer->emails()->get();

            // Vérifier si des emails existent
            if ($emails->isEmpty()) {
                Log::warning("Aucun email trouvé pour le client ID: $id");
                return redirect()->back()->with('error', 'Le client n\'a pas d\'adresse email.');
            }

            // Préparer un tableau contenant toutes les adresses email
            $emailAddresses = $emails->pluck('email')->toArray();

            // Envoi de l'email à toutes les adresses
            \App\Misc\Mail::setSystemMailDriver();

            \Mail::to($emailAddresses)->send(new SendEmailConnexion());

            // Mettre à jour la date d'envoi du mail
            $customer->email_sent_at = now();
            $customer->save();

            return redirect()->back()->with('success', 'Email envoyé avec succès à toutes les adresses.');

        } catch (\Exception $e) {
            Log::error("Erreur lors de l'envoi de l'email pour le client ID: $id", [
