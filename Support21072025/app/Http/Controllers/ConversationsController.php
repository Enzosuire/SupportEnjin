        ...
        switch ($request->action) {
            ...
            case 'send_reply':
                ...
                if (!$response['msg']) {
                    ...
                    // Récupération des emails secondaires du client
                    $emailsCC = [];

                    if (!empty($conversation->customer) && !empty($conversation->customer->emails)) {
                        foreach ($conversation->customer->emails as $emailObj) {
                            if (isset($emailObj['email'])) { // Vérifie si la clé email existe
                                $emailsCC[] = $emailObj['email'];
                            }
                        }
                    }

// Éviter d'ajouter les e-mails déjà en CC
                    $emailsCC = array_diff($emailsCC, $cc);

// Vérifier qu'il y a des emails avant de les ajouter en CC
                    if (!empty($emailsCC)) {
                        $thread->setCc(implode(',', $emailsCC)); // Ajoute les emails en CC
                    }
...
//                            $forwarded_conversation->setCc(array_merge(Conversation::sanitizeEmails($request->cc), [$to]));
        ...
        if (!empty($after_send)) {
            switch ($after_send) {
                ...
                case MailboxUser::AFTER_SEND_FOLDER:
                    ...
                    break;
                //Commenter pour ne plus avoir de redirection vers une conversation avec le status_active = statut nouveau
    ...
    public function processPhoneCustomer($request)
    {
        ...
        $request_last_name = '';
        $request_phone = '';
        if (trim($request->first_name ?? '') || trim($request->last_name ?? '') || trim($request->phone ?? '')) {
            $request_first_name = trim($request->first_name ?? '');
            $request_last_name = trim($request->last_name ?? '');
            $request_company = trim($request->company ?? '');
            $request_phone = trim($request->phone ?? '');

            $customer_data['first_name'] = $request_first_name;
            $customer_data['last_name'] = $request_last_name;
            $customer_data['company'] = $request_company;
            $customer_data['phones'] = [$request_phone];
        }

                // Check if name field contains ID of the customer.
        if (!$request->customer_id && is_numeric($request_first_name . $request_last_name . $request_company)) {
            // Try to find customer by ID.
            $customer = Customer::find($request_first_name . $request_last_name . $request_company);
        }
