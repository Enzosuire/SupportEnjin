...
<div id="editor_bottom_toolbar" style="display:none">
	...
	<select name="status" class="form-control parsley-exclude" data-reply-status="6" data-note-status="{{ $conversation->status }}">
        <option value="{{ App\Mailbox::TICKET_STATUS_ACTIVE }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_ACTIVE || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_STATUS_ACTIVE))selected="selected"@endif>{{ __('Nouveau') }}</option>
        <!-- Ajout TICKET_STATUS_NEW  qui correspond a en cours -->
        <option value="{{ App\Mailbox::TICKET_STATUS_NEW }}" @if ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_NEW  || ($mailbox->ticket_status == App\Mailbox::TICKET_STATUS_KEEP_CURRENT && $conversation->status == App\Mailbox::TICKET_STATUS_NEW ))selected="selected"@endif>{{ __('Active') }}</option>
