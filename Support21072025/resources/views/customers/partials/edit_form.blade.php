    ...
    <div class="row">
        <div class="col-xs-12">
            <form class="form-horizontal margin-top" method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-group{{ $errors->has('numero_siret') ? ' has-error' : '' }}">
                    <label for="numero_siret" class="col-sm-2 control-label">{{ __('Siren') }}</label>

                    <div class="col-sm-6">
                        <input id="numero_siret" type="text" class="form-control input-sized-lg" name="numero_siret" value="{{ old('numero_siret', $customer->numero_siret) }}" maxlength="255">

                        @include('partials/field_error', ['field'=>'numero_siret'])
                    </div>
                </div>
            ...
            <div class="" style="margin-top: 60px">
                <h2>Envoi de mail de connexion</h2>
                <div class="form-group{{ $errors->has('email_sent_at') ? ' has-error' : '' }}" style="margin-top: 20px">
                    <label for="email_sent_at" class="col-sm-2 control-label" style="text-align: right">{{ __('Envoi mail') }}</label>

                    <div class="col-sm-4">
                        @include('partials/field_error', ['field'=>'email_sent_at'])

                        <div class="mt-3">
                            @if(is_null($customer->email_sent_at))
                                <span>Aucun email de présentation du support n'a été envoyé à ce client.</span>
                            @else
                                <span>Dernier envoi : {{ $customer->email_sent_at }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="col-sm-4">
                        ...
                        <div class="mt-3">
                            <form method="POST" action="{{ route('customers.send-email', ['customer' => $customer->id]) }}">
                                {{ csrf_field() }}
