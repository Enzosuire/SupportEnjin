...
function initTimeTracking(timelog_dialog)
{
	$(document).ready(function(){
		...
		$('.timetr-log-value').click(function(e) {
			var span = $(this);
			span.hide();
			var time_spent = span.attr('data-time-spent');
			var html = '<div>';
			html += '<input type="number" value="'+timetrFormatTime(time_spent, 'hours', true)+'" class="form-control timetr-edit-value timetr-edit-h" /> : ';
			html += '<input type="number" value="'+timetrFormatTime(time_spent, 'minutes', true)+'" class="form-control timetr-edit-value timetr-edit-m" /> : ';
			html += '<input type="number" value="'+timetrFormatTime(time_spent, 'seconds', true)+'" class="form-control timetr-edit-value timetr-edit-s" /> &nbsp;';
			html += '<button class="btn btn-sm btn-primary timetr-edit-value timetr-save-timelog" type="button" data-loading-text="…"><i class="glyphicon glyphicon-ok"></i></button>';
			html += '</div>';

			$(html).insertAfter(span);

			$('.timetr-save-timelog').click(function(e) {
				timetrSaveTimelog($(this)[0]);
			});
		});
...
function timetrShowDialog(callback, cancel_callback)
{
	var modal_html = 
		'<div>'+
...
function timetrFormatTime(timestamp, hms, no_pad)
{
	var hours = Math.floor(timestamp / 3600);
    var minutes = Math.floor((timestamp / 60) % 60);
    var seconds = timestamp % 60;

    if (typeof(no_pad) == "undefined") {
...
function timetrSaveTimelog(el)
{
	var button = $(el);

	var h = Math.abs(parseInt(button.parent().children('.timetr-edit-h:first').val()));
	var m = Math.abs(parseInt(button.parent().children('.timetr-edit-m:first').val()));
	var s = Math.abs(parseInt(button.parent().children('.timetr-edit-s:first').val()));
	...
	var time_spent = h*3600 + m*60 + s;

	button.button('loading');
	fsAjax({
			action: 'edit',
			time_spent: time_spent,
			timelog_id: button.parents('tr:first').attr('data-timelog-id')
		}, 
		laroute.route('time_tracking.ajax'), 
		function(response) {
			button.button('reset');
			if (isAjaxSuccess(response)) {
				button.parent().prev()
					.attr('data-time-spent', time_spent)
					.text(timetrFormatTime(time_spent))
					.show();
