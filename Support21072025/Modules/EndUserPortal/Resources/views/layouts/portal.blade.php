	            ...
	            <div class="container">
	                ...
	                <div class="collapse navbar-collapse" id="app-navbar-collapse">
	                    ...
	                    <ul class="nav navbar-nav navbar-right">
							<li class="{{ \App\Misc\Helper::menuSelectedHtml('knowledgebase.frontend.home') }}"><a href="{{ route('knowledgebase.frontend.home', ['id' => \EndUserPortal::encodeMailboxId($mailbox->id)]) }}">{{ __('Bases de Connaissances') }}</a></li>
							...
							@if (EndUserPortal::authCustomer())
								...
								<li class="{{ \App\Misc\Helper::menuSelectedHtml('enduserportal.Projet.showportal') }}">
									<a href="{{ route('enduserportal.Projet.showportal', ['customer_id' => \EndUserPortal::authCustomer(), 'mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox->id)]) }}">
										{{ __('Projet') }}
									</a>
								</li>
								<li class="{{ \App\Misc\Helper::menuSelectedHtml('enduserportal.Facturations.showportal') }}">
									<a href="{{ route('enduserportal.Facturations.showportal', ['customer_id' => \EndUserPortal::authCustomer(), 'mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox->id)]) }}">
										{{ __('Facturation') }}
									</a>
								</li>
								<li class="{{ \App\Misc\Helper::menuSelectedHtml('enduserportal.Interventions.showportal') }}">
									<a href="{{ route('enduserportal.Interventions.showportal', ['customer_id' => \EndUserPortal::authCustomer(), 'mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox->id)]) }}">
										{{ __('Intervention') }}
									</a>
								</li>
							@endif
	                    	...
	                    	@else
								<li class="dropdown">
                                    ...
                                    <a href="#" class="dropdown-toggle dropdown-toggle-icon dropdown-toggle-account" data-toggle="dropdown">
                                    	<i class="glyphicon glyphicon-user"></i> <span class="nav-user">{{ EndUserPortal::authCustomer()->getCompanyName() }}</span> <span class="caret"></span>
                                    </a>

                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="#" id="eup-logout-link">
                                                {{ __('Log Out') }}
                                            </a>

                                            <form id="eup-logout-form" action="{{ route('enduserportal.logout', ['id' => \EndUserPortal::encodeMailboxId($mailbox->id)]) }}" method="POST" style="display: none;">
                                                {{ csrf_field() }}
                                            </form>
                                        </li>
                                    </ul>
	                    	...
	                    	@endif
	                    	{{--@if (\EndUserPortal::getLocales($mailbox))
		                    	<li class="dropdown">
	                                <a href="#" class="dropdown-toggle dropdown-toggle-icon" data-toggle="dropdown" title="{{ __('Search') }}">
	                                    <i class="glyphicon glyphicon-globe"></i> <small class="kb-locale-name">{{ \Helper::getLocaleData(\EndUserPortal::getLocale())['name'] ?? '' }}</small> <span class="caret"></span>
	                                </a>

	                                <ul class="dropdown-menu">
			                    		@foreach(\EndUserPortal::getLocales($mailbox) as $locale)
			                    			<li @if ($locale == \EndUserPortal::getLocale()) class="active" @endif><a href="{{ \EndUserPortal::urlToSwitchLocale($locale) }}">{{ \Helper::getLocaleData($locale)['name'] ?? '' }}</a></li>
			                    		@endforeach
	                                </ul>
	                            </li>
	                    	@endif--}}
