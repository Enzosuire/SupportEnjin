...
class EndUserPortalController extends Controller
{
    ...
    public function settings($mailbox_id)
    {
        ...
        $settings = [
            //'locales' => $meta_settings['locales'] ?? $default_settings['locales'],
            'numbers' => $meta_settings['numbers'] ?? $default_settings['numbers'],
            ...
            'subject' => $meta_settings['subject'] ?? $default_settings['subject'],
    ...
    public function loginProcess(Request $request, $mailbox_id = null)
    {
        ...
        if ($validator->fails()) {
            return redirect()
                ->route('enduserportal.login', ['mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox->id)])
        ...
        // Vérifiez si l'email appartient à un client existant.
        $customer = Customer::getByEmail($email);
        if (!$customer) {
            return redirect()
                ->route('enduserportal.login', ['mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox->id)])
                ->withErrors([
                    'email' => __('Cet email n\'est pas enregistré chez nous. Veuillez contacter le support si vous êtes un nouveau client.')
                ])
                ->withInput();
        }
        ...
        try {
            ...
            \Mail::to([['email' => $email]])->send(new \Modules\EndUserPortal\Mail\Login($mailbox, $customer));

            $result['message'] = __('Email with the authentication link has been sent to <strong>:email</strong>', ['email' => htmlspecialchars($email)]);
        } catch (\Exception $e) {
            \Helper::logException($e, '[End-User Portal]');
            $result['result'] = 'error';
            $result['message'] = __('Error occurred while sending email to <strong>:email</strong>', ['email' => htmlspecialchars($email)]);
        }
    ...
    //     $custom_errors = \Eventy::filter('enduserportal.login.custom_check', [], $request);
     ...
     * Login from email.
     */
    public function loginFromEmail(Request $request, $mailbox_id, $customer_id, $hash, $timestamp)
    {
        $result = [
            'result' => 'error',
        ...
        try {
            ...
            //  l'authentification est gérée pour le module portail utilisateurs  redirection vers le Modules portail utlisateur
            $auth_redirect = \EndUserPortal::authenticate($customer_id, $mailbox->id, $hash, $timestamp);

            //  l'authentification est gérée pour le module de la base de connaissance redirection vers le Modules base de connaissances
    ...
    public function tickets(Request $request, $mailbox_id = null)
    {
        ...
        // Début de la requête pour les tickets
        $ticketsQuery = Conversation::where('mailbox_id', $mailbox->id)
        ...
        if ($status) {
            switch ($status) {
                case 'new':
                    $ticketsQuery->where('status', Conversation::STATUS_NEW);
                    break;
                case 'active':
                    $ticketsQuery->where('status', Conversation::STATUS_ACTIVE);
                    break;
                case 'open':
                    $ticketsQuery->where('status', Conversation::STATUS_OPEN);
                    break;
                case 'pending':
                    $ticketsQuery->where('status', Conversation::STATUS_PENDING);
                    break;
                case 'closed':
                    $ticketsQuery->whereIn('status', [Conversation::STATUS_CLOSED]);
                    break;
            }
        }

        // Reste du code de votre méthode tickets() inchangé
        $tickets = $ticketsQuery->get();

        // Remove unneeded types (par exemple, TYPE_EMAIL)
        ...
        // Charger les threads pour les conversations et déterminer les nouveaux messages
        if (count($tickets)) {
                ...
                foreach ($tickets as $i => $ticket) {
                    ...
                    foreach ($threads as $thread) {
                        if ($ticket->id == $thread->conversation_id) {
                            ...
                            // Mettre à jour l'aperçu du ticket
        ...
        // Trier les tickets par nouvelles réponses et dernière réponse
    ...
    public function submitProcess(Request $request, $mailbox_id)
    {
        ...
        $custom_errors = \Eventy::filter('enduserportal.submit_form.custom_check', [], $request);

        if ($validator->fails() || !empty($request->age) || $custom_errors) {
            foreach ($custom_errors as $error_field => $error_message) {
                $validator->errors()->add($error_field, $error_message);
            }
    ...
    public function submitReply(Request $request, $mailbox_id, $conversation_id)
    {
        ...
        $custom_errors = \Eventy::filter('enduserportal.submit_reply.custom_check', [], $request);
    ...
    public function submitMessage($request, $mailbox_id)
    {
        ...
        if (empty($request->conversation_id)) {
            ...
            if (!$conversation) {
                ...
                // Make sure that conversation belongs to the current customer.
                if ($conversation->customer_id != $customer->id) {
                    return false;
                }
            }
        ...
        if ($new) {
            ...
            $subject = $request->subject ?: Conversation::subjectFromText($request->message);
    ...
    public function show($customer_id, $mailbox_id = null)
    {
        // Traiter l'identifiant de la boîte aux lettres
        $mailbox = $this->processMailboxId($mailbox_id);
        ...
        $authCustomer = \EndUserPortal::authCustomer();

        // Vérifier si l'utilisateur est authentifié et s'il correspond à l'id_customer dans l'URL
        if (!$authCustomer || $authCustomer->id != $customer_id) {
            // Redirige l'utilisateur vers la page de login avec le mailbox_id approprié
            return redirect()->route('enduserportal.login', ['mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox_id)]);
        }

        // Récupérer les projets associés à l'ID du client via la table de liaison
        $projets = Projet::whereIn('id', function ($query) use ($customer_id) {
            $query->select('id_projet')
                ->from('customer_projet')
        ...
        // Retourner la vue avec les projets regroupés et la boîte aux lettres
        return view('enduserportal::Projet.showportal', compact('mailbox', 'groupedProjets', 'projets'));
    }

    //Facturations
    public function showfa($customer_id, $mailbox_id = null)
    {
        // Traiter l'identifiant de la boîte aux lettres
        $mailbox = $this->processMailboxId($mailbox_id);
        ...
        // Récupérer les projets associés à ce client via la table de liaison customer_projet
        $projets = Projet::whereIn('id', function ($query) use ($customer_id) {
            $query->select('id_projet')
                ->from('customer_projet')
                ->where('id_customers', $customer_id);
        ...
        // Récupérer les IDs des projets associés au client
        $projetIds = $projets->pluck('id');

        // Récupérer les facturations liées à ces projets
        $facturations = Facturations::whereIn('id_projet', $projetIds)->get();

        // Regrouper les facturations par projet
        $groupedFacturations = $facturations->groupBy('id_projet');

        // Calculer les totaux par projet
        $totauxParProjet = $groupedFacturations->map(function ($facturations, $projetId) {
            return [
                'totalMontant' => $facturations->sum('montant'),
                'totalFacturations' => $facturations->count(),
            ];
        });

        // Retourner la vue avec les facturations, les projets, et la boîte aux lettres
        return view('enduserportal::Facturations.showportal', compact('mailbox', 'facturations', 'projets', 'groupedFacturations', 'totauxParProjet'));
    }
    ...
    public function showInt($customer_id, $mailbox_id = null, $id_projet = null)
    {
        // Traiter l'identifiant de la boîte aux lettres
        $mailbox = $this->processMailboxId($mailbox_id);
        ...
        // Vérifier si l'utilisateur est authentifié et correspond à l'id_customer dans l'URL
        if (!$authCustomer || $authCustomer->id != $customer_id) {
            return redirect()->route('enduserportal.login', ['mailbox_id' => \EndUserPortal::encodeMailboxId($mailbox_id)]);
        }
        ...
        // Récupérer toutes les interventions pour les projets du client
        ...
        $tempsRestantContrat = max(0, $projets->sum(function ($projet) {
        ...
        foreach ($interventionsDuClient->groupBy('id_projet') as $projetId => $interventionsProjet) {
            ...
            $tempsAlloueProjet = $interventionsProjet->sum(function ($intervention) {
        ...
        return view('enduserportal::Interventions.showportal', compact(
            'interventionsDuClient',
            'tempsRestantContrat',
            'projets',
            'typesinterventions',
            'tempsRestantParProjet',
