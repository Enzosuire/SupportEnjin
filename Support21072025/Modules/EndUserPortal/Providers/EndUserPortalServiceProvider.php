...
class EndUserPortalServiceProvider extends ServiceProvider
{
    ...
    //Méthode original sans le en cours 
    ...
    public static function getStatusName($conversation)
    {
        if ($conversation->status === Conversation::STATUS_ACTIVE && 
            ...
            return __('Nouveau'); // ou __('En Cours') pour afficher en français
        } 
        elseif ($conversation->status === Conversation::STATUS_NEW && 
                $conversation->state != Conversation::STATE_DELETED
        ) {
            return __('Active'); // ou un autre terme si souhaité
        } 
        elseif ($conversation->status === Conversation::STATUS_OPEN && 
                $conversation->state != Conversation::STATE_DELETED
        ) {
            return __('Ouvert'); // ou un autre terme si souhaité
        } 
        elseif ($conversation->status === Conversation::STATUS_PENDING && 
                $conversation->state != Conversation::STATE_DELETED
        ) {
            return __('Pending'); // ou un autre terme si souhaité
        } 
        else {
            return __('Closed');
        }
