			...
			function(response) {
				if (isAjaxSuccess(response)) {
					if (typeof(response.msg_success) != "undefined") {
						...
						html += ' '+htmlEscape(response.name)+' <small>(ID: '+customer_field_id+')</small>';
...
function crmCfInitMultiselect()
{
	$('.crm-cf-multiselect:visible').each(function(index, el) {
		var input = $(this);
		if (input.data('select2')) {
			return;
		}
			...
			function(response) {
				if (isAjaxSuccess(response)) {
					if (typeof(response.msg_success) != "undefined") {
						button.button('reset');
						showFloatingAlert('success', response.msg_success);
						// Redirection directe vers /clients
						window.location.href = '/clients';  // URL directe
						window.parent.postMessage('crm.refresh');
						setTimeout(function(){ 
